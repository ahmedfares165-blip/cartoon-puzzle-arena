...
function publicRoom(room) {
  return { code: room.code, image: room.image, startedAt: room.startedAt, players: [...room.players.values()].map(p => ({ name: p.name })) };
}
...
io.on('connection', socket => {
  socket.on('create-room', ({ name }) => {
    ...
    const room = { code, image: images[Math.floor(Math.random() * images.length)], startedAt: Date.now(), players: new Map() };
  ...
  socket.on('finish', ({ moves, seconds }) => {
    ...
    if (!room || room.players.get(socket.id)?.finished) return;
